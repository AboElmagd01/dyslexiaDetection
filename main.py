...
while True:
    ...
    for rect in rects:
        ...
        faceMinX = sys.maxsize  # these variables find the minimum and maximum values for X and Y, for both left and right eyes
        faceMaxX = -sys.maxsize - 1  # this is necessary to crop a box around each eye
        faceMinY = sys.maxsize
        faceMaxY = -sys.maxsize - 1
        ...
        for (ex, ey, ew, eh) in eyes:
            lis.append(ex + int(ew / 1.8))
            lis.append(ey + int(eh / 1.8))
        # cv2.imwrite("/mnt/Beta/pyCharm/dyslexia/faceFrame.jpg", justFace)
        #
        # returnValue = os.system(
        #     "/home/abdelrhman/Downloads/eyeLike-master/build/bin/eyeLike /mnt/Beta/pyCharm/dyslexia/faceFrame.jpg")
        # if (returnValue == -1):
        #     print("this didn't work")
        if(len(lis)>4):
            (rightEyeCenterX, rightEyeCenterY, leftEyeCenterX, leftEyeCenterY) = lis[0],lis[1],lis[2],lis[3]
        (rightEyeCenterX, rightEyeCenterY, leftEyeCenterX, leftEyeCenterY) = 0,0,0,0
        ...
        print('rightEyeCenterX, (rightEyeProgressionY[0]+ReLcY)', rightEyeCenterX + ReLcX,
              (rightEyeProgressionY[0] + ReLcY))
